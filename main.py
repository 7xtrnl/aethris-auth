...
DB_PATH          = os.getenv("DB_PATH", "aethris.db")
login_attempts: dict[str, list[float]] = defaultdict(list)
RATE_LIMIT_MAX   = 5
RATE_LIMIT_WINDOW = 60
BOT_ACTION_QUEUE: list[dict] = []

app = FastAPI(title="Aethris Auth", version="2.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def generate_key(prefix: str = "AETHRIS") -> str:
...
class ManageRoleRequest(BaseModel):
    discord_id: str
    role_id:    str
    action:     str
...
async def redeem_key(req: RedeemKeyRequest):
    ...
    try:
        ...
        if row["is_used"]:
            raise HTTPException(400, "Key already redeemed")
        if c.execute("SELECT id FROM users WHERE username=?", (req.username,)).fetchone():
            raise HTTPException(400, "Username already taken")

        expiry = None if row["is_lifetime"] else (
            datetime.utcnow() + timedelta(days=row["expiry_days"])
        ).isoformat()

        c.execute(
            "INSERT INTO users (username,password_hash,password_plain,discord_id,discord_name,"
            "expiry_date,is_lifetime,created_at) VALUES (?,?,?,?,?,?,?,datetime('now'))",
            (req.username, hash_password(req.password), req.password,
             req.discord_id, req.discord_name, expiry, 1 if row["is_lifetime"] else 0)
        )
        uid = c.lastrowid
        c.execute(
            "UPDATE license_keys SET is_used=1, redeemed_at=datetime('now'), user_id=? WHERE key=?",
            (uid, row["key"])
        )
        conn.commit()
...
async def login(req: LoginRequest, request: Request):
    ...
    try:
        ...
        if u["is_banned"]:
            raise HTTPException(403, f"Banned: {u['ban_reason']}")
        if not u["is_lifetime"] and u["expiry_date"]:
            if datetime.utcnow() > datetime.fromisoformat(u["expiry_date"]):
                raise HTTPException(403, "Subscription expired")
        if u["hwid"] and u["hwid"] != req.hwid:
            raise HTTPException(403, "HWID mismatch. Contact support.")
        if not u["hwid"]:
            c.execute("UPDATE users SET hwid=? WHERE id=?", (req.hwid, u["id"]))
        token = secrets.token_hex(32)
        c.execute(
            "UPDATE users SET auth_token=?, last_login=datetime('now'), last_ip=? WHERE id=?",
            (token, ip, u["id"])
        )
        conn.commit()
        return {
            ...
            "token":       token,
            "username":    u["username"],
            "expiry":      u["expiry_date"] or "Lifetime",
...
async def user_info(discord_id: str):
    ...
    try:
        ...
        return {
            ...
            "hwid_bound":  bool(u["hwid"]),
            "is_banned":   bool(u["is_banned"]),
            "expiry_date": u["expiry_date"] or "Lifetime",
            "is_lifetime": bool(u["is_lifetime"]),
...
async def user_reset_hwid(req: UserResetHWIDRequest):
    ...
    try:
        ...
        conn.commit()
        return {"success": True, "message": "HWID reset. You can now log in from a new device."}
    finally:
        conn.close()
...
async def user_reset_password(req: UserResetPasswordRequest):
    ...
    try:
        ...
        if u["is_banned"]:
            raise HTTPException(403, "Account is banned")
        c.execute(
            "UPDATE users SET password_hash=?, password_plain=? WHERE discord_id=?",
...
async def admin_keys(_=Depends(require_admin)):
    ...
    try:
        rows = c.execute("""
            SELECT lk.*, u.username, u.discord_id, u.discord_name,
                   u.password_plain, u.hwid, u.expiry_date,
                   u.last_login, u.last_ip, u.is_banned
...
async def admin_generate(
    ...
    expiry_days: int  = Query(30, ge=1),
    lifetime:    bool = Query(False),
    ...
    try:
        ...
        for _ in range(count):
            k = generate_key()
            c.execute(
                "INSERT INTO license_keys (key, is_lifetime, expiry_days) VALUES (?,?,?)",
                (k, 1 if lifetime else 0, expiry_days)
            )
            keys.append(k)
        conn.commit()
        return {"generated": count, "keys": keys}
    finally:
        conn.close()
...
async def admin_reset_password(req: ResetPasswordRequest, _=Depends(require_admin)):
    ...
    try:
        r = c.execute(
            "UPDATE users SET password_hash=?, password_plain=? WHERE username=?",
            (hash_password(req.new_password), req.new_password, req.username)
        )
        conn.commit()
        if r.rowcount == 0:
            raise HTTPException(404, "User not found")
...
async def admin_extend_key(req: ExtendKeyRequest, _=Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        return {"success": True, "new_expiry": new_exp}
    finally:
        conn.close()
...
async def bot_poll(_=Depends(require_admin)):
    actions = BOT_ACTION_QUEUE.copy()
    BOT_ACTION_QUEUE.clear()
    return actions
